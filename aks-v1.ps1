...
echo "Run command: Set-Variable -Name AzureRegistryID -Value (az acr show --resource-group myrg --name acrbexarcountydemo --query ""id"")"
echo "`n"
Set-Variable -Name AzureRegistryID -Value $(az acr show --resource-group myrg --name acrbexarcountydemo --query "id")
echo "OUTPUT"
$AzureRegistryID
Write-Output "--------------------------------------------------------------------"
echo "`n"
...
echo "Run command: kubectl create secret docker-registry acr-auth --docker-server acrbexarcountydemo.azurecr.io --docker-username ServicePrincipalID --docker-password ServicePrincipalPassword --docker-email joydeep.ghosh@us.sogeti.com"
echo "`n"
echo "OUTPUT"
kubectl create secret docker-registry acr-auth --docker-server acrbexarcountydemo.azurecr.io --docker-username $ServicePrincipalID --docker-password $ServicePrincipalPassword --docker-email akhterul.mustafa@us.sogeti.com
Write-Output "--------------------------------------------------------------------"
echo "`n"
